...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      window.Stripe = () => ({ initCheckoutElementsSdk: options => { if ('email' in options.defaultValues) throw new Error('Customer email is already set'); return ({ loadActions: async () => ({ type: 'success', actions: { getSession: () => ({ currency: 'eur', minorUnitsAmountDivisor: 100, canConfirm: true, total: { total: { minorUnitsAmount: 2490 } } }), confirm: async options => { if ('email' in options) throw new Error('Customer email is already set'); return { type: 'error', error: { message: 'Test: Zahlung abgelehnt' } }; } } }), on: () => {}, createPaymentElement: () => element('Sichere Zahlungsfelder'), createExpressCheckoutElement: () => element('Wallets') }); } });
    });
