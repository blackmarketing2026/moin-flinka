...
export async function openNativeCheckout(result, order, form, onEdit) {
  ...
  const checkout = window.Stripe(result.publishableKey).initCheckoutElementsSdk({
    ...
    // Contact details are already stored on the server-created Stripe customer.
  ...
  try {
    ...
    async function confirm(event) {
      ...
      try {
        const confirmed = await actions.confirm({ redirect: 'if_required', billingAddress: { name: order.name, address: { line1: order.street, postal_code: order.postcode, city: order.town, country: 'DE' } }, ...(event ? { expressCheckoutConfirmEvent: event } : {}) });
