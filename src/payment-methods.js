const methods = [
  ['Stripe', '<text x="6" y="29" fill="#635bff" font-size="20" font-weight="800">stripe</text>'],
...
if (!document.querySelector('.admin-page')) {
  ...
  for (const [name, graphic] of methods.filter(([name]) => ['Kreditkarte', 'Google Pay', 'Apple Pay', 'Amazon Pay'].includes(name))) { const item = document.createElement('span'); item.className = 'payment-method'; item.innerHTML = `<svg viewBox="0 0 68 44" role="img" aria-label="${name}">${graphic}</svg>`; const caption = document.createElement('small'); caption.textContent = name; item.append(caption); list.append(item); }
...
const checkoutIcons = document.querySelector('.checkout-payment-icons');
if (checkoutIcons) {
  checkoutIcons.classList.add('payment-method-icons');
  for (const name of ['Amazon Pay', 'Google Pay', 'Stripe', 'Amex', 'Kreditkarte', 'Mastercard', 'Maestrocard']) {
    const graphic = methods.find(([label]) => label === name)[1];
    const item = document.createElement('span'); item.className = 'payment-method';
    item.innerHTML = `<svg viewBox="0 0 68 44" role="img" aria-label="${name}">${graphic}</svg>`;
    const caption = document.createElement('small'); caption.textContent = name; item.append(caption); checkoutIcons.append(item);
  }
}
